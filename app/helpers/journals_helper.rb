module JournalsHelper
  ...
  def dis_episode(item,ck_type,link: true,prompt_new: false)
    if ck_type[6].nil?
      ...
      item.episode_id ? (link ? link_to(ck_type[2],episode_url(item.episode_id)) : ck_type[2]) : (prompt_new ? link_to("提交申请",new_episode_path(holiday_id: ck_type[6],journal_id: item.id)) : "无假条")
    end
  end
