class UsersController < ApplicationController
  ...
  def kaoqing
    ...
    _is_expired = !@date.between?(*Journal.count_time_range(is_for_validate: true))
    ...
    @rule = AttendRule.find_by(id: @user.try(:leader_data).try(:[],"attend_rule_id")) #rescue (Rails.logger.debug{"error_no_leader_data:#{@user.id}"};raise CanCan::AccessDenied.new("非部门管理员",home_users_path))
  ...
  private
    ...
    def user_params
      ...
      params.require(:user).permit(:uid, :password,:mgr_code,:title,:password_confirmation,:dept_code,:work_date,:onboard_date, :expire_date,role_ids: [],department: [] )
    end
