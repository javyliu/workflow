class UsersController < ApplicationController
  ...
  def home
    ...
    @user = current_user.decorate
    @my_journals = current_user.journals.where("check_type = 10 or dval != 0").order("update_date desc,id desc").page(params[:page]).select("journals.*,checkin,checkout").joins("inner join checkinouts on update_date = rec_date and journals.user_id = checkinouts.user_id ")
